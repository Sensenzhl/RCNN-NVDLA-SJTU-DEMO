...
range_outdir="${workspace}../amod_tests/datadistri/bvlc_rcnn/log_range"
data_outdir="${workspace}../amod_tests/datadistri/bvlc_rcnn/log_data"
...
mkdir -p ${range_outdir} ${data_outdir}

cp ${config} ${range_outdir}
cp ${config} ${data_outdir}
# Collect the data range of each blob
${workspace}amod_scripts/submit_jobs.pl -model_def ${config} -pre_trained ${model} -maxtime ${maxtime} -out_dir ${range_outdir} -skip 0 -interval 2 -histdir $histdir -hist_mode 0 -db DLA_FAST_SIMU
...
${workspace}amod_scripts/merge_hist.py --model ${config} --file_num ${hist_file_num} --dir ${histdir} --log range_collect.log
# Collect the histogram of each blob
${workspace}amod_scripts/submit_jobs.pl -model_def ${config} -pre_trained ${model} -maxtime ${maxtime} -out_dir ${data_outdir} -skip 0 -interval 2 -histdir $histdir -hist_mode 1 -db DLA_FAST_SIMU
# submit the failed tasks again
